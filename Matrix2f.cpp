...
const Matrix2f Matrix2f::operator*(const Matrix2f& m) const{
	Matrix2f out = Matrix2f(*this);
	float p1 = out[0][0]*(m[1][0] - m[1][1]);
	float p2 = m[1][1]*(out[0][0] + out[1][0]);
	float p3 = m[0][0]*(out[0][1] + out[1][1]);
	float p4 = out[1][1]*(m[0][1] - m[0][0]);
	float p5 = (out[0][0] + out[1][1]) * (m[0][0] + m[1][1]);
	float p6 = (out[1][0] - out[1][1]) * (m[0][1] + m[1][1]);
	float p7 = (out[0][0] - out[0][1]) * (m[0][0] + m[1][0]);
	out[0][0] = p5 + p4 - p2 + p6;
	out[0][1] = p3 + p4;
	out[1][0] = p1 + p2;
	out[1][1] = p1 + p5 - p3 - p7;
